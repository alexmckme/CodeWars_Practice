...
function stickyCalc (operation, val1, val2){
    let roundVal1 = Math.round(val1)
    let roundVal2 = Math.round(val2)
    let interNumber = roundVal1.toString() + roundVal2.toString()
    interNumber = Number(interNumber)
    if (operation === "+") return Math.round(interNumber + roundVal2);
    if (operation === "-") return Math.round(interNumber - roundVal2);
    if (operation === "*") return Math.round(interNumber * roundVal2);
    if (operation === "/") return Math.round(interNumber / roundVal2);
}